...
async def create_product(message, MCategory, categories, media_path, alert):
    ...
    if message:
        try:
            ...
            pcQty = check_value(re.sub('\D', '', RefinedTxt[3]))
            if pcQty:
                pcQty = int(pcQty)
            price = re.sub('[^\d|^\d.\d]', '', RefinedTxt[4])
            if price:
                price = float(price)
            pcPrice = re.sub('\D', '', RefinedTxt[5])
            if pcPrice:
                pcPrice = int(pcPrice)
            true = True
            false = False
            ...
            ***REMOVED***,
                ...
                "enabled": true,
                "options": OpBody,
                "description": f"<b>Choose from our collection of quality Turkish brands for {nameEn}. We offer you the largest selection of Turkish kids {nameEn} and the latest trends in childern's {main_category_en} fashion that suit all tastes. In different sizes and colours.</b>",
